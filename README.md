...
- **16 套排版主题**：内置暖陶米白（仿 Claude 风格）、经典蓝、杂志衬线、极简黑白、靛粉渐变、极客深色、优雅棕、活力红、琥珀橙、清新绿、健康绿、杂志红、复古纸、夜空蓝、暖阳红、海盐青。
- **字号与段距微调**：可在主题默认样式基础上调整字号和段落间距，设置会同步影响预览、复制到公众号和发公众号草稿。
...
- 在预览面板点击 **更多 -> 复制 Markdown**。
- 或在命令面板执行 **复制当前笔记 Markdown**。

插件会优先复制当前编辑器中的内容，便于在保存前也拿到最新 Markdown。
...
配置完成后，可以在预览面板点击 **发公众号草稿**，或在命令面板执行 **发公众号草稿**。弹窗中可确认标题、作者、摘要和封面图，随后插件会执行以下流程：
